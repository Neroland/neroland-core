...
/**
 * Functional screen for the Trash Can: a bevelled hull panel with a recessed cell
 * drawn behind every slot (the single trash drop slot up top + the full player
 * inventory + hotbar below), so the menu reads like a normal container instead of a
 * bare backdrop. The trash slot shows the last item dropped in (the next one to be
 * voided). Uses the 26.x container-screen API
 * ({@code extractContents(GuiGraphicsExtractor, ...)}); no texture asset.
 */
