package za.co.neroland.nerolandcore.menu;
...
/**
 * Trash Can menu — a single drop slot. Items placed in it are NOT trashed
 * immediately: the trash can holds the last-inserted stack and only voids it when
 * the NEXT stack is inserted, so closing the screen never trashes the held stack.
 * The backing container (the block entity) always reports the slot empty, so every
 * insert is accepted and routed through the hold-then-void buffer.
 */
